...
class HealthcareInsuranceSubscription(Document):
    ...
    def validate_insurance_company(self):
        ...
        if not contract:
            frappe.throw(_('No valid contract found for this Insurance Company {0}').format(self.insurance_company))
    ...
    def validate_subscription_overlap(self):
        ...
        if insurance_subscription:
            frappe.throw(_('Patient {0} has already subscribed for this coverage plan {1}').format(frappe.bold(self.patient),
