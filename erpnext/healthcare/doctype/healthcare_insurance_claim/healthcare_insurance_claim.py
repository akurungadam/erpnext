...
class HealthcareInsuranceClaim(Document):
	...
	def set_insurance_coverage(self):
		...
		if not coverage_detail:
			...
			if self.mode_of_approval == 'Automatic':
				# mode_of_approval cannot be automatic if claim not based on coverage, i.e., manual
	...
	def set_insurance_price_list_rate(self):
		...
		price_list_detail = None

		if insurance_price_lists and insurance_price_lists.get('plan_price_list'):
			price_list_rate = get_item_price_list_rate(self.item_code, insurance_price_lists.get('plan_price_list'), self.qty, self.company)
			if price_list_rate:
				price_list_detail = {'price_list': insurance_price_lists.get('plan_price_list'), 'price_list_rate': price_list_rate}

		elif insurance_price_lists and insurance_price_lists.get('default_price_list'):
			price_list_rate = get_item_price_list_rate(self.item_code, insurance_price_lists.get('default_price_list'), self.qty, self.company)
			if price_list_rate:
				price_list_detail = {'price_list': insurance_price_lists.get('default_price_list'), 'price_list_rate': price_list_rate}

		else:
			# Fall back to Default Selling Price List
			selling_price_list = frappe.db.get_single_value('Selling Settings', 'selling_price_list')

			if selling_price_list:
				price_list_rate = get_item_price_list_rate(self.item_code, selling_price_list, self.qty, self.company)
				if price_list_rate:
					price_list_detail = {'price_list': selling_price_list, 'price_list_rate': price_list_rate}

		if price_list_detail and price_list_detail.get('price_list_rate'):
